...
// this function is used by the ticker to flip back and forth whether the
// temperture pressure event flag was set
void flip() {
...
// this function takes in the two addresses that hold 8 bits and reads the
// information from them, and then combinds them into a 16 bit number
...
void read_pressure() {
  ...
  while (true) {
    ...
    int32_t UP =
        ((output0[0] << 16) + (output1[0] << 8) + output2[0]) >> (8 - oss);
...
int main() {
  ...
  flipper.attach(&flip, 3.0); // the address of the function to be attached
